...
var (
	networkCard   = flag.String("network", "", "Enter network adapter description, use 'auto' for automatic selection")
	port          = flag.Int("port", 8989, "Default API port")
	expireTime    = flag.Int64("expire", 10, "Data expiration time (seconds), default 10s")
	autoCheckTime = flag.Int("autoCheckTime", 3, "Auto-check active network adapter time (seconds)")
)
...
func main() {
	...
	if err != nil {
		log.Fatal("Failed to get network adapters: ", err.Error())
	}
	if deviceName == "auto" {
		log.Println("Auto-detecting active network adapter, please wait...")
		active := ncap.GetActiveNetworkCards(devices, *autoCheckTime)
		if active != nil {
			log.Println("Automatically found suitable network adapter: ", active.Desc)
			log.Println("Monitored packet count: ", active.PacketCount)
			log.Println("Monitored traffic: ", fmt.Sprintf("%d bytes (%.2f KB)", active.ByteCount, float64(active.ByteCount)/1024))
	...
	if deviceName == "" {
		...
		prompt := &survey.Select{
			Message: "Unable to auto-detect active network adapter, please manually select (you can find the adapter description in your network settings):",
			Options: options,
		}
		...
		if len(option) == 0 {
			log.Fatalf("Selected network adapter is empty")
		}
		deviceName = option
	}
	...
	s.GET("/api/enemies", func(ctx *gin.Context) {
		...
		for id, item := range global.SceneMonsterList {
			if time.Now().Unix()-item.UpdateTime > et {
				continue // Ignore data not updated in the past 10 seconds
			}
			attackPlayers := make(map[uint64]*global.AttackPlayer)
			if item.AttackPlayers != nil {
				for uid, player := range item.AttackPlayers {
					if time.Now().Unix()-player.LastAttackTime > et { // Ignore players who haven't participated in combat in the past 10 seconds
						continue
					}
