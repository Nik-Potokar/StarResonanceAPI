...
// GetActiveNetworkCards gets the network adapter currently in use
func GetActiveNetworkCards(devices []pcap.Interface, autoCheckTime int) *InterfaceStats {
	if len(devices) == 0 {
		log.Fatal("No network adapters found")
	}
	checkTime := autoCheckTime
	if 1 > checkTime {
		checkTime = 3
	}
	log.Println(fmt.Sprintf("Starting to monitor all network adapter traffic, please wait %d seconds", checkTime))
...
func monitorInterface(deviceName string, stats *InterfaceStats, done chan bool) {
	// Open network adapter for packet capture
	handle, err := pcap.OpenLive(deviceName, 1600, true, pcap.BlockForever)
	if err != nil {
		// Some adapters may not open, silently ignore
		return
	}
	defer handle.Close()
